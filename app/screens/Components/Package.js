...
export const calculateTotal = weight => {
      ...
      <tbody>
        ...
        </tr>
        {
			props.packages && props.packages.map((item, index) => {
				return (<tr key={`${index}-${item.price}`} className="d-flex">
  							...
  							<td className="col-3">
    							<ul>{item.items.map((packItmem, i, all) => <li key={`${index}-${packItmem.name}`}>{`${packItmem.name}`}{i >= all.length-1 ? '': ','}</li>)}</ul>
  							</td>
						</tr>)})
		}
      </tbody>
